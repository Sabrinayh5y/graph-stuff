...
import { Canvas, resizeObserver, throttle, ruler, Cardinal, animation } from "vislite"
...
// 刻度尺
let rulerData = ruler(maxValue, 0, 5, {})
...
// 注册鼠标移动事件
let hasCurrent: boolean, hadInit = false, itemWidth: number

let updateView: Function
class LineMultipleXAxis extends React.Component {
    ...
    render() {
        return (<div className='LineMultipleXAxis-view'>

            {/* 绘制图表 */}
            <div ref={this.refContent} className="content" onMouseMove={event => this.doHover.call(this, event)}></div>
    ...
    doResize() {
        ...
        for (let i = 0; i < data[0].value.length; i++) {
            let x = (i + 0.5) * ((mycontent.clientWidth - grid.left - grid.right) / data[0].value.length) + grid.left
        ...
        // 生成插值函数实例
        let cardinalTop = new Cardinal().setP(pointsTop)
        let cardinalBottom = new Cardinal().setP(pointsBottom)

        updateView = (deep: number, hoverData: any) => {
            if (!this.painter) return

            this.painter.clearRect(0, 0, mycontent.clientWidth, mycontent.clientHeight)
            ...
            for (let i = 0; i < rulerData.length; i++) {
                let y = calcY(rulerData[i])

                this.painter.fillText(rulerData[i], grid.left - 5, y)
            ...
            for (let i = 0; i < data[0].value.length; i++) {
                let x = (i + 0.5) * itemWidth + grid.left

                this.painter.fillText(data[0].year + "-" + (i + 1), x, grid.top - 15)
                this.painter.beginPath().moveTo(x, grid.top).lineTo(x, grid.top - 5).stroke()
            }
            ...
            for (let i = 0; i < data[1].value.length; i++) {
                let x = (i + 0.5) * itemWidth + grid.left
                let y = mycontent.clientHeight - grid.bottom

                this.painter.fillText(data[1].year + "-" + (i + 1), x, y + 15)
            ...
            for (let x = grid.left; x < (mycontent.clientWidth - grid.left) * deep + grid.left - grid.right; x += 5) {
                this.painter.lineTo(x, cardinalTop.use(x))
            }
            this.painter.stroke()
            ...
            for (let x = grid.left; x < (mycontent.clientWidth - grid.left) * deep + grid.left - grid.right; x += 5) {
                this.painter.lineTo(x, cardinalBottom.use(x))
            }
            this.painter.stroke()
...
export default LineMultipleXAxis